...
6. `from typing import Callable, Iterator` 这一行**整行删掉**。两者都只被刚删掉的
   旧函数用过 —— `main()` 的签名是 `def main(argv: list[str] | None = None) -> int`，
   `emit` 是 `def emit(event: dict) -> None`，都不涉及它们。删完用
   `uv run python -c "import chrometrans.cli"` 确认没有 NameError。
...
Run: `uv run pytest -q`
Expected: **143 passed**。基线是 140：迁走 `segment_source` 那条 −1，`test_pipeline.py`
新增 4 条。（`test_cli.py` 里的 `publish` 用例还没动，那是 Step 11 的事。）
...
4. `main()` 里那句 `from chrometrans.server import EventBus, create_app` 改成
   只导入 `EventBus` —— `create_app` 只被删掉的 `_serve()` 用过
5. 把 `main()` 里那一段改成：
...
Run: `uv run pytest -q`
Expected: **143 passed**。迁走 `publish` 那条 −1，`test_serving.py` 新增 1 条，
与 Step 6 的 143 持平。
...
- Produces:
  ...
  - `.add(cue: dict) -> bool`（`cue` 就是 `cue` 事件里的 `data`，即 `Cue.to_dict()`；
    返回值是**是否挤掉了最旧的那一条** —— 控件靠它决定要不要跟着裁自己那份文档）
  - `.cues() -> list[dict]`
  - `.__len__() -> int`
...
- Produces:
  ...
  - `CaptionWindow(cap: int = 300, parent=None)`，含 `add_cue(cue: dict) -> None`、`clear_cues() -> None`、`toPlainText() -> str`、`toHtml() -> str`、`set_font_px(px: int) -> None`、`font_px() -> int`、`set_opacity(v: float) -> None`、`opacity() -> float`、`apply_settings(s: GuiSettings) -> None`、`current_settings() -> GuiSettings`
...
- Produces:
  ...
  - 方法 `refresh() -> None`、`set_status(text: str, degraded: bool = False) -> None`、`set_capturing(bool) -> None`、`selected() -> AudioProcess | None`、`status_text() -> str`、`hint_text() -> str`、`labels() -> list[str]`、`select_pid(pid: int) -> None`、`start_enabled() -> bool`、`click_start() -> None`、`open_page_enabled() -> bool`、`set_page_option_enabled(enabled: bool) -> None`、`page_option_is_enabled() -> bool`
  - 常量 `REFRESH_MS = 2000`、`EMPTY_HINT`、`ERROR_HINT_PREFIX`
  - 上面这些方法里，`labels` / `select_pid` / `start_enabled` / `click_start` / `open_page_enabled` / `set_page_option_enabled` / `page_option_is_enabled` 是**测试要用的观察与驱动入口**，不是内部实现细节 —— 它们必须存在且行为如测试所述。
